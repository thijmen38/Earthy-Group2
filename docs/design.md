...
The manual inputs apart from the excel sheet used for the script are the site dimension (length and breadth), direction of north with respect to site and to place the entrance checkpoint on site.

The script sorts the data from the excel into its assigned branches of hierarchy, creates rectangular regions in the North, South, East, West, NorthEast, SouthEast directions of the site and populates it with the number spaces assigned to it.

The entrance checkpoint is aligned and snapped to the grid on site, this geometry's center is used as the attractor point to dynamically relaxing the spaces in depth 1 hierarchy. Dynamic relaxtion is a principal that showcases a spring like behaviour. Therefore, in the set of goals a certain strength of tension is created between the entrance checkpoint and the spaces which is released constantly upon enabling dynamic relaxation. The relaxed curved are then snaped to the grid and assigned a colour as per code. 
...
Here is a gif showing the spatial configuration results of this computational process. 
